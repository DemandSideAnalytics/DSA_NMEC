...
cap program drop dsa_seasonal_dowt
version 16.0
program dsa_seasonal_dowt, rclass

/*
	Purpose: 	Computes a seasonal Day-of-Week & Temperature (DOWT) model
				with an optional synthetic control. The included variables
				are a 7-bin temperature spline, 7 day-of-week dummy 
				variables and any optional granular profiles. Regressions are
				run independently for each season: Summer, Winter, and Shoulder.
		
	Assumed structure:
		data long by id(s) date
		id(s) | date | treatment| temperature | profiles
...
*A. Define the syntax
syntax varlist(min=2 max=2) [if], id(varlist) date(varlist) 				///
...
*D. Keep the relevant variables only 
	keep `id' `date' `usage' `tempf' `treatment' `profiles'
...
*F. Create analysis variables and flag relevant buckets
	...
	*02. create dow variable (1-7) (Monday through Sunday)
	gen 	_dow 	= dow(`date')
	...
	*04. Start Seasonal Loop
	forval s = 1/3 { 
		...
		* Loop through the bins and fill in the temperature bins	
		...
		/*=============================================================*
			regress using caltrack method on pre period
		*=============================================================*/
		...
		forval p = 1/`_maxid' {
			...
			cap { 	// for cases where there is no data in that month for a
				...
				* Run Regression
				reg `usage' ibn._dow  c._temp_bin_*	 `profiles' 		///
		...
		* Keep values of interest & clean up 
		...
		keep `id' `date' _season _month _training _post 					///
...
*H. Merge the predicted results back to the original file
	use `original', clear	
	merge 1:1 `id' `date' using `results', nogen
	gen _analysis = _training == 1 | _post == 1
	
	
*I. Label the analysis variables
	label var _analysis 		"Seasonal DOWT: Training and Reporting Period"
	label var _training 		"Seasonal DOWT: Observations used to train the model"
	label var _post 			"Seasonal DOWT: Post-treatment period"
	label var _season 			"Seasonal DOWT: Season used for regression analysis"
	label var _month 			"Seasonal DOWT: Month"
	label var _counterfactual 	"Seasonal DOWT: Model Counterfactual"
		
	if "`keepknots'" != "" {
		forval b = 1/7 {
			label var _b`b'_lb		"Seasonal DOWT: Bin `b' Lower Bound"
			label var _b`b'_ub		"Seasonal DOWT: Bin `b' Upper Bound"
			label var _b`b'_valid	"Seasonal DOWT: Bin `b' Valid Bin"
		}
		label var _high_bin			"Seasonal DOWT: Highest Valid Temp Bin"
	}		
